  ...
  const [result] = await executor.execute(
    `
      INSERT INTO notifications (
        user_id, shipment_id, notification_type, title, message,
        channel, sent_at, metadata
      )
...
const createNotifications = async (notifications, connection = null) => {
  ...
  for (const notification of notifications.filter((item) => item?.userId)) {
    created.push(await createNotification(notification, connection));
  }
...
const listNotifications = async (
  ...
  const [countRows] = await executor.execute(
    `
      ...
      INNER JOIN users u ON u.id = n.user_id AND u.deleted_at IS NULL
...
const listActiveUserIdsByRole = async (role, connection = null) => {
  const executor = getExecutor(connection);
